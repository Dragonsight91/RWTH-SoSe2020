...
\begin{document}
    ...
    \begin{tikzpicture}[->,>=stealth',shorten >=2pt,auto,node distance=3.5cm,
        ...
        \path (q_0) edge    [loop above] node[text width=1cm,align=center] {$a,a | aaa$\\$a,\Gamma_0|\Gamma_0 aa$} (q_0)
              (q_1) edge    [loop above] node {$b,a | \epsilon$} (q_1)
              (q_1) edge              node {$\epsilon, \Gamma_0 | \Gamma_0$} (q_2)
              (q_0) edge              node {$b,a | \epsilon$} (q_1);
